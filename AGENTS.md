...
- Follow `editor_generator/oscillator_card.rs`: a narrow left identity rail, then contiguous oscillator, unison, and pan panels separated by subtle vertical dividers. Use `editor_theme::space`, `shape`, `graph_inset`, and `compact_gap`; no local spacing scale.
