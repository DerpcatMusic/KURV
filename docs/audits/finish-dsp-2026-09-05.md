...
- `experimental-1x-dsp` is off by default. It enables a finite-harmonic high-note saw/triangle crossover for unwarped canonical shapes. Configured host modulation and generator routes use the established antialiaser, including zero-depth routes; custom and warped paths retain fallback behavior. This does not certify arbitrary host automation or FM/PM sidebands. Existing unused event/correction experiments were not imported. Repeated warp guards reuse one helper.
