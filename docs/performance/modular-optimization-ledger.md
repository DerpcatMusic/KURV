...
### R0061 - Reject global derivative-bound mip selection for phase warps

- Temporary file tested and removed: `examples/warp_mip_lab.rs`
- Hypothesis: select a custom-wave mip from the oscillator step multiplied by
  a proven upper bound on each phase warp's absolute derivative. This keeps
  every retained source harmonic's worst instantaneous phase rate below
  Nyquist without adding work to the table evaluator.
- Production bandlimit source SHA-256:
  `bbe1d8aaa387cd8da3e7ecd5ce7b0f90f487cedaa01684315c818835f226455f`
- Probe binary SHA-256:
  `b2d58b8c50d2591a41d492f7da9d1c570d9628c67bcf7bf1e1add94316a07fb6`
- The release probe measured 224 cells: two source curves, no warp plus PWM,
  phase-bend, and harmonic warps, five depths, and seven coherent pitch bins.
  Derivative-bounded selection improved alias residual in 81 cells and
  regressed it in 12, for a mean 6.308 dB alias reduction.
- That reduction came from selecting much duller source tables. Total spectral
  error improved in only one cell and regressed in 106, averaging 3.922 dB
  worse. Wanted-spectrum error worsened by 27.557 dB on average. The largest
  alias reduction was 76.903 dB, but that cell's total error worsened from
  -28.612 to -16.296 dBc because legal wanted harmonics disappeared.
- The worst raw-warp alias cell moved only from -8.500 to -9.590 dBc while
  total error worsened from -8.414 to -7.102 dBc. This also demonstrates that
  filtering the source before a nonlinear phase map cannot suppress all
  sidebands generated by the warp itself.
- Decision: rejected and removed in full. Keep raw-step table selection for
