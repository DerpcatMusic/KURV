...
def output(**overrides):
    values = dict(CASE, callbacks=256, repeats=5, warmup_callbacks=256, finite="true", peak=0.2,
...
class PerformanceMatrixTests(unittest.TestCase):
    def test_full_matrix_covers_required_axes(self):
        ...
        self.assertEqual({case["voices"] for case in cases}, {1, 8, 32})
    ...
    def test_axes_are_configurable_and_independent(self):
        cases = list(runner.matrix("full", [31, 256], [44100, 96000], [1, 4],
                                   voices=[3, 32], unison=[1, 64]))
        names = {case["scenario"] for case in cases}
        self.assertTrue({"xpm-1x64", "xpm-64x1", "xcyclepm-64x64", "xdepthpm-64x64"} <= names)
        self.assertEqual({case["voices"] for case in cases}, {3, 32})
        self.assertEqual({case["frames"] for case in cases}, {31, 256})
        self.assertEqual(len(cases), len({tuple(sorted(case.items())) for case in cases}))
        overridden = list(runner.matrix("full", [64], [48000], [1], [32], [64], ["xpm-1x64"]))
        self.assertEqual([case["scenario"] for case in overridden], ["xpm-1x64"])

    def test_held_note_and_unison_limits_are_distinct(self):
        common = ["runner", "base", "candidate", "--dry-run", "--scenarios", "xpm-1x64"]
        with patch("sys.argv", common + ["--voices", "32", "--unison", "64"]), contextlib.redirect_stdout(io.StringIO()):
            self.assertEqual(runner.main(), 0)
        for invalid in [["--voices", "33"], ["--unison", "65"]]:
            with patch("sys.argv", common + invalid), contextlib.redirect_stderr(io.StringIO()), self.assertRaises(SystemExit):
                runner.main()
    ...
    def test_command_has_independent_notes_lanes_and_oversampling(self):
        self.assertEqual(runner.command(Path("/bin/lab"), CASE, 256, 5),
                         ["/bin/lab", "64", "256", "5", "solo-saw-1", "1", "48000", "2", "256"])
    ...
    def test_runner_alternates_and_records_binary_metadata(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            args = ["runner", "--baseline-build", "base rustc flags", "--candidate-build", "candidate rustc flags", str(baseline), str(candidate), "--rounds", "3", "--output", str(destination)]
    ...
    def test_failed_run_is_saved_and_never_reported_as_speedup(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            destination = Path(directory)/"report.json"
            args = ["runner", "--baseline-build", "base rustc flags", "--candidate-build", "candidate rustc flags", str(binary), str(binary), "--output", str(destination)]
            with patch("sys.argv", args), patch.object(runner, "matrix", return_value=iter([CASE])), \
