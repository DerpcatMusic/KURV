...
from probe import ROOT, BASE, method, production_constants, compiler_args, compiler_command
...
def main():
    p=argparse.ArgumentParser();p.add_argument('--candidate-root',type=pathlib.Path,default=ROOT);compiler_args(p);a=p.parse_args()
...
'''.replace('// PRODUCTION_CAPACITIES',production_constants(a.candidate_root))+ '\nmod baseline {use super::*;'+module(old)+'}\nmod candidate {use super::*;'+module(new)+'}\n'
    with tempfile.TemporaryDirectory(prefix='kurv-depth-eligibility-') as d:
        ...
        subprocess.run(compiler_command(a)+['--edition=2024','-C','opt-level=3',str(src),'-o',str(binary)],check=True)
