...
## KURV vendored patches

This copy contains the audited Linux editor-close fallback (handler opt-in, callback revocation, and successful ELF lifetime pinning before bounded detach) and bounded X11 XDND v5 `text/uri-list` target support. Arbitrary handlers retain synchronous close. Both patches have focused tests under `src/x11`.
