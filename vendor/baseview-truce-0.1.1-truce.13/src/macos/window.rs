...
/// Permanently pin the Mach-O image containing baseview/KURV before detached
/// work can resume after the host destroys the plug-in instance.
pub fn pin_current_image_for_detached_work() -> bool {
    static PINNED: std::sync::OnceLock<bool> = std::sync::OnceLock::new();
    *PINNED.get_or_init(|| {
        // Apple's dyld uses 0x80 for RTLD_NODELETE. libc intentionally does
        // not expose the named constant on every macOS SDK target.
        const RTLD_NODELETE_FLAG: libc::c_int = 0x80;
        let mut info = std::mem::MaybeUninit::<libc::Dl_info>::zeroed();
        let symbol = pin_current_image_for_detached_work as *const () as *const libc::c_void;
        // SAFETY: `info` is writable and `symbol` names code in this image.
        if unsafe { libc::dladdr(symbol, info.as_mut_ptr()) } == 0 {
            return false;
        }
        // SAFETY: dladdr initialized `info`; the loader owns `dli_fname` for
        // the immediately following call. The successful handle is leaked on
        // purpose so NODELETE remains effective for process lifetime.
