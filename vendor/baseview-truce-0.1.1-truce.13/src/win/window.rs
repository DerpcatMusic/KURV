...
use windows_sys::Win32::{
    ...
    UI::{
        ...
        WindowsAndMessaging::{
            AdjustWindowRectEx, DefWindowProcW, DestroyWindow, DispatchMessageW, GetDesktopWindow,
            GetMessageW, GetParent, KillTimer, LoadCursorW, PostMessageW, SendMessageW, SetCursor,
            SetParent, SetTimer, SetWindowPos, ShowWindow, TranslateMessage, HTCLIENT, MSG,
            SWP_NOACTIVATE, SWP_NOMOVE, SWP_NOZORDER, SW_HIDE, WHEEL_DELTA, WM_CHAR, WM_CLOSE,
...
impl WindowHandle {
    pub fn close(&mut self) {
        if let Some(hwnd) = self.hwnd.take() {
            // Host GUI destruction must not return while author callbacks and
            // an unpinned renderer can still be live. Send synchronously; the
            // bounded close path either joins or safely parks before return.
            unsafe {
...
unsafe fn wnd_proc_inner(
    ...
    match msg {
        ...
        WM_CLOSE => {
            ...
            // Route every ordinary close through the bounded renderer teardown
            // path instead of letting DefWindowProc destroy the HWND first.
        ...
        BV_WINDOW_MUST_CLOSE => {
            // Quiesce author entry points before asking the handler to shut
            // down independently blocked renderer work.
            window_state.stop_frame_timer();
            let _ = RevokeDragDrop(hwnd);
            ShowWindow(hwnd, SW_HIDE);
            // A child HWND must be parked before renderer detachment. SetParent
            // returns the previous (non-null) host parent on success. If it
            // fails, the handler synchronously joins rather than allowing work
            // to outlive an HWND the host can destroy.
            let previous_parent = GetParent(hwnd);
            let parked =
                previous_parent.is_null() || !SetParent(hwnd, GetDesktopWindow()).is_null();
            match window_state.detach_renderer_for_bounded_close(parked) {
                Some(true) => {
                    // The handler pinned its image and detached only its
                    // callback-free renderer. Drop author callbacks now. The
                    // renderer posts this message again after exit; with no
                    // handler left, that pass destroys the parked HWND.
                    let handler = window_state.handler.borrow_mut().take();
                    drop(handler);
                    Some(0)
                }
                Some(false) => {
                ...
                None => {
                    // A close requested from inside an author callback is
                    // reentrant. Let that callback return and retry instead of
                    // destroying the handler while its RefCell borrow is live.
                    PostMessageW(hwnd, BV_WINDOW_MUST_CLOSE, 0, 0);
                    Some(0)
                }
            }
...
impl WindowState {
    ...
    fn stop_frame_timer(&self) {
        // Idempotently revoke the ordinary SetTimer fallback too.
        unsafe {
            KillTimer(self.hwnd, WIN_FRAME_TIMER);
        }
    ...
    fn detach_renderer_for_bounded_close(&self, window_will_remain_valid: bool) -> Option<bool> {
        let Ok(mut handler) = self.handler.try_borrow_mut() else {
            return None;
        };
        Some(handler.as_mut().is_some_and(|handler| {
            handler.detach_renderer_for_bounded_close(window_will_remain_valid)
        }))
    }
...
/// Permanently pin the DLL containing baseview/KURV before detached work can
/// resume through it after the host destroys the plug-in instance.
pub fn pin_current_image_for_detached_work() -> bool {
    ...
    *PINNED.get_or_init(|| {
        ...
        let address = pin_current_image_for_detached_work as *const () as *const u16;
        // SAFETY: FROM_ADDRESS makes `address` an address inside the desired
        // module rather than a string pointer. PIN prevents its refcount from
        // ever reaching zero for the remainder of the process.
        unsafe {
            GetModuleHandleExW(
...
/// Post final destruction back to the thread that created a parked HWND.
pub fn close_detached_window_when_safe(hwnd: isize) {
    if hwnd != 0 {
        // SAFETY: posting is valid cross-thread; stale HWND values simply make
        // PostMessageW fail without dereferencing process memory.
        unsafe {
            PostMessageW(hwnd as HWND, BV_WINDOW_MUST_CLOSE, 0, 0);
        }
    }
}
