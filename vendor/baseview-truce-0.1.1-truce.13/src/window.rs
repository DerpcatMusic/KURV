...
pub trait WindowHandler {
    /// Opt in to Linux's callback-revoking, lifetime-pinned bounded-close
    /// fallback. Only audited renderer handlers should override this.
    ...
    /// Windows-only close preparation for handlers with independently blocked
    /// renderer threads. Returning `true` asks baseview to revoke/drop the
    /// handler but park the hidden HWND until that renderer posts its exit.
    #[cfg(target_os = "windows")]
    fn detach_renderer_for_bounded_close(&mut self, window_will_remain_valid: bool) -> bool {
        let _ = window_will_remain_valid;
        false
    }
...
/// Pin the current plug-in image before background work may outlive its owner.
///
/// The pin is intentionally process-lifetime. Callers must reject detachment
/// when this returns `false`.
#[cfg(any(target_os = "linux", target_os = "windows", target_os = "macos"))]
pub fn pin_current_image_for_detached_work() -> bool {
    platform::pin_current_image_for_detached_work()
}

/// Ask the creator thread to destroy a parked detached-renderer window after
/// its final renderer has exited.
#[cfg(target_os = "windows")]
pub fn close_detached_window_when_safe(hwnd: isize) {
    platform::close_detached_window_when_safe(hwnd);
}
