...
use truce_core::bus::{BusKind, ChannelConfig};
...
/// Distinct stopped-transport parameter IDs retained latest-wins until the next
/// process call. This exceeds KURV's complete host parameter set by an order of
/// magnitude while keeping the process-side merge fixed and allocation-free.
const DEFERRED_PARAM_CAPACITY: usize = 4096;
...
struct ClapAudio<P: PluginExport> {
    ...
    /// Parameter events delivered while processing is stopped. CLAP applies
    /// their atomic values in `params_flush`; retaining their identities until
    /// the next process block lets plug-ins distinguish an explicit same-value
    /// host event from an unchanged parameter snapshot.
    deferred_param_events: EventList,
...
/// Merge stopped-transport parameter events ahead of the current live block.
/// Both lists are preallocated by the wrapper; the batch rotation is linear,
/// allocation-free, and preserves chronology for repeated parameter IDs.
fn merge_deferred_param_events(live: &mut EventList, deferred: &mut EventList) {
    let mut appended = 0;
    for event in deferred.iter().copied() {
        if live.len() >= EVENT_LIST_PREALLOC + DEFERRED_PARAM_CAPACITY {
            break;
        }
        live.push(event);
        appended += 1;
    ...
    deferred.clear();
    live.rotate_last_to_front(appended);
}
...
unsafe extern "C" fn clap_plugin_process<P: PluginExport>(
    ...
    let result = run_audio_block_with::<P, i32>("CLAP", CLAP_PROCESS_ERROR, || unsafe {
        ...
        if state_loaded {
            // These events predate the recalled state, just like the live
            // events discarded by `convert_input_events` above.
            scr.deferred_param_events.clear();
        } else {
            merge_deferred_param_events(&mut scr.event_list, &mut scr.deferred_param_events);
        }
    ...
    run_extern_callback_with::<P, ()>("CLAP", "params_flush", (), || unsafe {
        ...
        for ev in scr.event_list.iter() {
            if let EventBody::ParamChange { id, value } = ev.body {
                ...
                let _ = scr
                    .deferred_param_events
                    .push_latest_param_change(deferred, DEFERRED_PARAM_CAPACITY);
            }
        }
...
unsafe extern "C" fn state_load<P: PluginExport>(
    ...
    run_extern_callback_with::<P, bool>("CLAP", "load_state", false, || unsafe {
        ...
        // Read one bounded host-state envelope. The limit is shared with
        // KURV's RESYNTH state contract: reject before Vec growth rather than
        // trusting an unbounded or misbehaving host stream.
        ...
        loop {
            ...
            if n > buf.len()
                || blob
                    .len()
                    .checked_add(n)
                    .is_none_or(|total| total > MAX_HOST_STATE_BYTES)
            {
                return false;
            }
...
fn audio_port_main_flag(kind: BusKind) -> u32 {
    if kind == BusKind::Main {
...
unsafe extern "C" fn audio_ports_get<P: PluginExport>(
    ...
    unsafe {
        ...
        out.flags = audio_port_main_flag(bus.kind);
...
unsafe extern "C" fn audio_ports_config_get<P: PluginExport>(
    ...
    unsafe {
        ...
        let main_input = layout.inputs.iter().find(|bus| bus.kind == BusKind::Main);
        out.has_main_input = main_input.is_some();
        out.main_input_channel_count = main_input.map_or(0, |bus| bus.channels.channel_count());
        out.main_input_port_type =
            main_input.map_or(ptr::null(), |bus| clap_port_type_ptr(bus.channels));
        let main_output = layout.outputs.iter().find(|bus| bus.kind == BusKind::Main);
        out.has_main_output = main_output.is_some();
        out.main_output_channel_count = main_output.map_or(0, |bus| bus.channels.channel_count());
        out.main_output_port_type =
            main_output.map_or(ptr::null(), |bus| clap_port_type_ptr(bus.channels));
        true
    }
}
    ...
    run_extern_callback_with::<P, *const clap_plugin>("CLAP", "create", ptr::null(), || {
        ...
        let data = Box::new(ClapPluginData::<P> {
            ...
            audio: PluginCell::new(ClapAudio {
                event_list: EventList::with_capacity(
                    EVENT_LIST_PREALLOC + DEFERRED_PARAM_CAPACITY,
                ),
                sounding_notes: SoundingNotes::new(midi_input_ports),
                output_events: EventList::with_capacity(EVENT_LIST_PREALLOC),
                deferred_param_events: EventList::with_capacity(DEFERRED_PARAM_CAPACITY),
                sub_event_scratch: EventList::with_capacity(
                    EVENT_LIST_PREALLOC + DEFERRED_PARAM_CAPACITY,
                ),
                sample_rate: 44100.0,
                max_block_size: 1024,
...
mod bus_kind_tests {
    ...
    #[test]
    fn sidechain_port_is_not_reported_as_clap_main() {
        assert_eq!(audio_port_main_flag(BusKind::Sidechain), 0);
        assert_eq!(audio_port_main_flag(BusKind::Main), CLAP_AUDIO_PORT_IS_MAIN);
    }
}


#[cfg(test)]
mod deferred_param_event_tests {
    use super::*;

    fn param(offset: u32, id: u32, value: f64) -> Event {
        Event::new(offset, EventBody::ParamChange { id, value })
    }

    #[test]
    fn stopped_events_are_latest_wins_and_precede_live_block_events() {
        let mut deferred = EventList::with_capacity(DEFERRED_PARAM_CAPACITY);
        assert!(deferred.push_latest_param_change(param(0, 7, 0.2), DEFERRED_PARAM_CAPACITY));
        assert!(deferred.push_latest_param_change(param(0, 7, 0.6), DEFERRED_PARAM_CAPACITY));
        assert_eq!(deferred.len(), 1);

        let mut live =
            EventList::with_capacity(EVENT_LIST_PREALLOC + DEFERRED_PARAM_CAPACITY);
        live.push(param(0, 8, 0.1));
        live.push(param(32, 7, 0.7));
        merge_deferred_param_events(&mut live, &mut deferred);

        let changes: Vec<_> = live
            .iter()
            .map(|event| match event.body {
                EventBody::ParamChange { id, value } => (event.sample_offset, id, value),
                _ => unreachable!(),
            })
            .collect();
        assert_eq!(changes, vec![(0, 7, 0.6), (0, 8, 0.1), (32, 7, 0.7)]);
