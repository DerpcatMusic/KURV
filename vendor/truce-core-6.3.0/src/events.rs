...
impl EventList {
    ...
    /// Retain the newest value for a parameter without growing beyond
    /// `max_events`. Intended for stopped-transport wrapper queues where
    /// replaying an older same-ID value would be incorrect.
    pub fn push_latest_param_change(&mut self, event: Event, max_events: usize) -> bool {
        let EventBody::ParamChange { id, .. } = event.body else {
            return false;
        };
        if let Some(existing) = self.events.iter_mut().find(|existing| {
            matches!(existing.body, EventBody::ParamChange { id: existing_id, .. } if existing_id == id)
        }) {
            *existing = event;
            return true;
        }
    ...
    /// Move the last `count` events to the front while preserving order within
    /// both ranges. Format wrappers use this after appending parameter events
    /// that chronologically predate the current process block.
    ///
    /// This is allocation-free and linear. Only event records move; SysEx pool
    /// offsets remain valid because the payload arena is unchanged.
    pub fn rotate_last_to_front(&mut self, count: usize) {
        let count = count.min(self.events.len());
        self.events.rotate_right(count);
    }
...
mod tests {
    ...
    #[test]
    fn latest_param_change_replaces_same_id_without_growing() {
        let mut list = EventList::with_capacity(2);
        assert!(list.push_latest_param_change(
            Event::new(0, EventBody::ParamChange { id: 7, value: 0.2 }),
            2,
        ));
        assert!(list.push_latest_param_change(
            Event::new(0, EventBody::ParamChange { id: 7, value: 0.8 }),
            2,
        ));
        assert_eq!(list.len(), 1);
        assert!(matches!(
            list.iter().next().unwrap().body,
            EventBody::ParamChange { id: 7, value } if value == 0.8
        ));
    }

    #[test]
    fn rotate_last_to_front_preserves_both_ranges_and_sysex_offsets() {
        let mut list = EventList::with_capacity(8);
        list.push_sysex(4, b"\x10\x11").unwrap();
        list.push(Event::new(
            8,
            EventBody::ParamChange { id: 1, value: 0.7 },
        ));
        list.push(Event::new(
            0,
            EventBody::ParamChange { id: 2, value: 0.5 },
        ));
        list.rotate_last_to_front(1);
        let events: Vec<_> = list.iter().copied().collect();
        assert!(matches!(events[0].body, EventBody::ParamChange { id: 2, .. }));
        assert_eq!(list.sysex_bytes(&events[1].body), b"\x10\x11");
        assert!(matches!(events[2].body, EventBody::ParamChange { id: 1, .. }));
    }
