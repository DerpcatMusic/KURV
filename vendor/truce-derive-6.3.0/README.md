...
This copy adds the opt-in `#[persist_missing]` companion attribute. After a structurally valid keyed document is parsed, only an opted-in absent `#[persist]` field receives an empty cursor. KURV uses that hook on `generator-stack` to recognize pre-modular state; other custom fields and an absent overall persist block retain their existing semantics.
