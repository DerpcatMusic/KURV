...
/// Whether an absent key should be reported to this custom field through an
/// empty cursor. This is opt-in because empty input has no generic
/// `PersistField` meaning and existing implementations may interpret it as a
/// reset or malformed document.
...
/// Read struct-level Params behavior. `post_load` runs after parameter values
/// and keyed persistent fields have both been restored, so migrations can
/// reconcile data that historically lived in either representation.
    ...
    for attr in attrs {
        ...
        attr.parse_nested_meta(|meta| {
            if meta.path.is_ident("id_scheme") {
                ...
                let value: syn::LitStr = meta.value()?.parse()?;
                parsed.post_load = Some(syn::parse_str::<syn::Ident>(&value.value()).map_err(|_| {
                    meta.error("`post_load` must name an inherent method on the Params struct")
                })?);
            } else {
                return Err(meta.error(
                    "unknown `#[params]` key (expected `id_scheme` or `post_load`)",
                ));
            }
            Ok(())
        })?;
...
pub fn derive_params(input: TokenStream) -> TokenStream {
    ...
    let post_load = struct_attrs.post_load;
    ...
    let persist_seen_idents: Vec<_> = persist_fields
        .iter()
    ...
    let persist_read_arms: Vec<proc_macro2::TokenStream> = persist_fields
        ...
        .map(|((ident, key, _), seen)| {
            quote! {
                #key => {
                    #seen = true;
                    ::truce::core::custom_state::PersistField::persist_read(
                        &self.#ident,
        ...
        .collect();
    let persist_seen_declarations = persist_seen_idents
        .iter()
        .map(|seen| quote! { let mut #seen = false; });
    let persist_missing_stmts =
        persist_fields
            .iter()
            .zip(&persist_seen_idents)
            .filter_map(|((ident, _, migrate_missing), seen)| {
                migrate_missing.then(|| {
                    quote! {
                        if !__data.is_empty() && !#seen {
                            ::truce::core::custom_state::PersistField::persist_read(
                                &self.#ident,
                                &mut ::truce::core::custom_state::StateCursor::new(&[]),
                            );
                        }
                    }
                })
    ...
    let post_load_stmt = post_load.map(|method| quote! { self.#method(); });
