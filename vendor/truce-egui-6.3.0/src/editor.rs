...
impl DragDropInput {
    ...
    fn drop_files(&mut self, data: baseview::DropData) -> bool {
        ...
        let accepted = !dropped.is_empty();
        self.dropped_files.extend(dropped);
        accepted
    }
...
impl<P: Params + ?Sized> EguiWindowHandler<P> {
    ...
    fn recover_device(&mut self, window: &mut Window) -> bool {
        ...
        #[cfg(target_os = "windows")]
        {
            // Device-loss callbacks may arrive just before the render thread
            // finishes unwinding. Do not replace or detach a generation still
            // using this HWND; retry on a later GUI tick after it reports exit.
            if self
                .render_thread
                .as_ref()
                .is_some_and(|render_thread| !render_thread.has_exited())
            {
                return false;
            }
    ...
    fn run_frame(&mut self, window: &mut Window) -> Option<std::time::Duration> {
        ...
        self.drag_drop.apply_to(&mut raw_input);
        raw_input
            .viewports
...
impl<P: Params + ?Sized + 'static> WindowHandler for EguiWindowHandler<P> {
    fn allow_bounded_close_detach() -> bool {
        true
    }

    #[cfg(target_os = "windows")]
    fn detach_renderer_for_bounded_close(&mut self, window_will_remain_valid: bool) -> bool {
        let park_window = self.render_thread.as_mut().is_some_and(|render_thread| {
            if window_will_remain_valid {
                render_thread.detach_for_bounded_close()
            } else {
                render_thread.join_synchronously_for_close();
                false
            }
        });
        // The thread either joined or detached after pinning. In the latter
        // case baseview has already parked the HWND before dropping this handler.
        self.render_thread = None;
        park_window
    }
    ...
    fn on_frame(&mut self, window: &mut Window) {
        ...
        let frame_result = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
            ...
            if self.device_lost.load(Ordering::Acquire) {
                if self.recover_device(window) {
                    log::warn!("egui device-loss recovery: rebuilt");
                    // The rebuilt surface starts blank; paint next frame even
            ...
            match event {
                Event::Mouse(mouse) => {
                    ...
                    match mouse {
                        ...
                        // CursorEntered is intentionally ignored; drag variants returned
                        // above through `handle_drag_event`.
        ...
        let window = baseview::Window::open_parented(
            ...
            move |window: &mut Window| {
                ...
                EguiWindowHandler::<P> {
                    ...
                    drag_drop: DragDropInput::default(),
...
mod tests {
    ...
    fn drag_adapter_state() -> (DragDropInput, Vec<egui::Event>, egui::Modifiers, egui::Pos2) {
        (
            DragDropInput::default(),
            Vec::new(),
            egui::Modifiers::NONE,
            egui::Pos2::ZERO,
        )
    ...
    #[test]
    fn drag_enter_populates_hovered_files_and_moves_pointer() {
        let paths = vec![PathBuf::from("first.wav"), PathBuf::from("second.wav")];
        let (mut drag_drop, mut events, mut modifiers, mut cursor_pos) = drag_adapter_state();
        let event = baseview::MouseEvent::DragEntered {
            position: baseview::Point::new(12.5, 24.25),
            modifiers: keyboard_types::Modifiers::SHIFT,
            data: baseview::DropData::Files(paths.clone()),
        };

        let status = handle_drag_event(
            &event,
            &mut drag_drop,
            &mut events,
            &mut modifiers,
        ...
        assert_eq!(
            status,
            Some(EventStatus::AcceptDrop(baseview::DropEffect::Copy))
        );
        assert_eq!(cursor_pos, egui::pos2(12.5, 24.25));
        assert!(modifiers.shift);
        assert_eq!(events, vec![egui::Event::PointerMoved(cursor_pos)]);
        assert_eq!(
            raw_input
                .hovered_files
                .iter()
    ...
    #[test]
    fn drag_move_replaces_hovered_files_and_moves_pointer() {
        let (mut drag_drop, mut events, mut modifiers, mut cursor_pos) = drag_adapter_state();
        drag_drop.hover(baseview::DropData::Files(vec![PathBuf::from("old.wav")]));
        let new_path = PathBuf::from("new.wav");
        let event = baseview::MouseEvent::DragMoved {
            position: baseview::Point::new(90.0, 45.0),
            modifiers: keyboard_types::Modifiers::CONTROL,
            data: baseview::DropData::Files(vec![new_path.clone()]),
        };
        ...
        assert_eq!(cursor_pos, egui::pos2(90.0, 45.0));
        assert!(modifiers.ctrl);
        assert_eq!(raw_input.hovered_files.len(), 1);
        assert_eq!(raw_input.hovered_files[0].path.as_ref(), Some(&new_path));
    }

    #[test]
    fn drag_leave_clears_hovered_files() {
        let (mut drag_drop, mut events, mut modifiers, mut cursor_pos) = drag_adapter_state();
        drag_drop.hover(baseview::DropData::Files(vec![PathBuf::from("hover.wav")]));
        ...
        assert_eq!(status, Some(EventStatus::Captured));
    ...
    #[test]
    fn drag_drop_clears_hovered_files_and_delivers_dropped_files_once() {
        let path = PathBuf::from("dropped.wav");
        let (mut drag_drop, mut events, mut modifiers, mut cursor_pos) = drag_adapter_state();
        drag_drop.hover(baseview::DropData::Files(vec![path.clone()]));
        let event = baseview::MouseEvent::DragDropped {
            position: baseview::Point::new(30.0, 60.0),
        ...
        let mut second_frame = egui::RawInput::default();
        drag_drop.apply_to(&mut second_frame);

        assert_eq!(
            status,
            Some(EventStatus::AcceptDrop(baseview::DropEffect::Copy))
        );
        assert_eq!(cursor_pos, egui::pos2(30.0, 60.0));
        assert!(modifiers.alt);
        assert!(first_frame.hovered_files.is_empty());
        assert_eq!(first_frame.dropped_files.len(), 1);
        assert_eq!(first_frame.dropped_files[0].path.as_ref(), Some(&path));
        assert!(second_frame.dropped_files.is_empty());
    }

    #[test]
    fn unsupported_drag_data_is_not_accepted() {
        let (mut drag_drop, mut events, mut modifiers, mut cursor_pos) = drag_adapter_state();
        let event = baseview::MouseEvent::DragEntered {
            position: baseview::Point::new(1.0, 2.0),
        ...
        assert!(drag_drop.hovered_files.is_empty());
    }
}
