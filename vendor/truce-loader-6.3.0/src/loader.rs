...
impl<S: Sample> NativeLoader<S> {
    ...
    fn copy_versioned(&mut self) -> Result<PathBuf, std::io::Error> {
        ...
        let stem = self
            ...
            .and_then(|s| s.to_str())
            .unwrap_or("plugin");
        // The loader counter is only process-scoped. Include the PID so
        // separate DAW/plugin-host processes cannot overwrite one another's
        // mapped hot-reload copy and fault the audio thread with SIGBUS.
        let process_id = std::process::id();
        let temp = std::env::temp_dir().join(format!(
            "truce-hot-{stem}-{}-{process_id}-{}.{ext}",
