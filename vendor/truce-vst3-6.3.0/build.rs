fn main() {
    ...
    if build.get_compiler().is_like_msvc() {
        build.flag("/std:c++17");
        // Match the crate's `+crt-static` rustflags. Mixing a /MD shim
        // with a statically-linked Rust cdylib imports VCRUNTIME140 and
        // can abort the host on process attach.
